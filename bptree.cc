...
copy_from_temp_to_right_parent(TEMP *temp, NODE *right)
{
	...
	right->nkey = 0;
	for (i=0; i<N-(int)ceil((N+1)/2)-1; i++)
	{
		right->key[i] = temp->key[(int)ceil((N+1)/2)+1+i];
		right->chi[i] = temp->chi[(int)ceil((N+1)/2)+1+i];
...
insert_temp_after_left_child(TEMP *temp, NODE *left_child, int rs_key, NODE *right_child)
{
	...
	for (i=0; i < temp->nkey; i++)
	{
		if (rs_key < temp->key[i]) break;
	}
	int keyid = i;
	int ptrid = i+1;

	//Shift
	for (int j=N-1; j>keyid; j--)
	{
		temp->key[j] = temp->key[j-1];
	}
	for (int j=N; j>ptrid; j--)
	{
		temp->chi[j] = temp->chi[j-1];
	}

	//Insert
	temp->key[keyid] = rs_key;
	temp->chi[ptrid] = right_child;
	temp->nkey++;
